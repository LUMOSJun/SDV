...
var DEFAULT_COLOR_END = "DarkBlue"
...
function filterData(Month) {
    ...
    */
    filtered_virus_data = virus_data.filter(function (obj) {
...
function printMap() {
    ...
    var countrynumbers = [];
    var keys = Object.keys(virus_data[0])

    // initialize the number winners to the first country
    Alpha_country = {
        name: virus_data[0].Country,
        numbers: +virus_data[0].Alpha
    }
    Delta_country = {
        name: virus_data[0].Country,
        numbers: +virus_data[0].Delta
    }
    Omicron_country = {
        name: virus_data[0].Country,
        numbers: +virus_data[0].Omicron
    }

    filtered_virus_data.forEach(element => {
        var numberCount = 0;
        for (var key of keys) {
            switch (key) {
                case 'Alpha':
                    numberCount += +element[key];
                    if (+element[key] > Alpha_country.numbers)
                        Alpha_country = {
                ...
                case 'Delta':
                    numberCount += +element[key];
                    if (+element[key] > Delta_country.numbers)
                        Delta_country = {
                            name: element.Country,
                            numbers: +element[key]
                        }
                ...
                case 'Omicron':
                    numberCount += +element[key];
                    if (+element[key] > Omicron_country.numbers)
                        Omicron_country = {
                            name: element.Country,
                            numbers: +element[key]
                        }
        ...
        countrynumbers.push(numberCount);
        numberCount = 0;
    });

    /*
        Update number winners list
    */
    $("#Alpha-number-country-text").text(Alpha_country.name + ": " + Alpha_country.numbers);
    $("#Delta-number-country-text").text(Delta_country.name + ": " + Delta_country.numbers);
    $("#Omicron-number-country-text").text(Omicron_country.name + ": " + Omicron_country.numbers);

    /*
        Create the color scale using the min and max
        of the total number counts for the filtered
        olympic data
    */
    var colorScale = d3.scaleLinear()
        .range([DEFAULT_COLOR_START, DEFAULT_COLOR_END])
        .domain([0, Math.max(...countrynumbers)]);
    ...
    */
    g.selectAll(".country")
        ...
        .style("fill", function (d) {
            /*
                Get the total number count for a
        ...
        .on("mouseover", function (data) {
            ...
            */
            ...
            /*
                if we found a country for the current Month
            */
            ...
            /*
                ...
                the current country and current Month selected
            */
            filtered_virus_data.forEach(element => {
            ...
            /*
                We found no current country for this Month
                so we only know the date and country name
                default the Alpha, Delta, Omicron to 0
            */
...
function ready(data) {
    ...
    */
    virus_data.forEach(function (d) {
        if (!map.has(d.Month)) {
            map.set(d.Month, [d.Country])
        } else {
            map.get(d.Month).push(d.Country);
        }
    });

    /* 
        Displays the Month selector 
    */
    var Month_option_select = d3.select('#dropdown-menu').append("select").attr("class", "Month-option-select");

    Month_option_select.selectAll("option")
        .data(map.keys())
        .enter()
        .append("option")
    ...
    */
    ...
    /*
        draw default map
    */
    d3.select(window).on("resize", throttle(Month));
    ...
    */
    ...
    Month_option_select.on("change", function () {
        /* 
            The menu has been changed, now grab the Month from the drop down menu 
        selected = true;
            DONT THINK WE NEED THIS ANYMORE?
        */

        /* 
            Redraw the map with the selected Month 
        */
        Month = $("#dropdown-menu").find(".Month-option-select").val()
        redraw(Month);
    });
}
...
function throttle(desired_Month) {
    window.clearTimeout(throttleTimer);
    throttleTimer = window.setTimeout(function () {
        redraw(desired_Month);
    }, 200);
}
